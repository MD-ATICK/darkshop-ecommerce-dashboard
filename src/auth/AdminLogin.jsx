	...
	return auth.loading ? (
		...
		<div className=' w-full h-screen flex items-center flex-col bg-gray-500 p-4 lg:p-10'>
			<div className='flex justify-between w-full bg-transparent items-center'>
				...
				<Link
					to='/register'
					className=' text-sky-700 cursor-pointer text-[15px] tracking-[0.020em] bg-transparent'
				>
					Register
				</Link>
			</div>
			<form
				onSubmit={adminLoginHanlder}
				className='max-w-[500px] mt-10 w-full flex flex-col gap-y-6 bg-white rounded-2xl shadow-lg p-6 md:p-9'
			>
				<h1 className='text-[26px] font-bold text-stone-900'>
					<span className='text-sky-600 text-[38px]'>D</span>-Shop ~ Admin Login
				</h1>
				<div className='flex gap-1 flex-col-reverse'>
					{auth.status === 222 && auth.data.error.includes("account") && (
						<p className='text-red-500 text-[12px] tracking-wide'>
				...
				<div className='flex gap-1 flex-col-reverse'>
					{auth.status === 222 && auth.data.error.includes("password") && (
						<p className='text-red-500 text-[12px] tracking-wide'>
				...
				<div className='flex gap-x-3 items-center'>
					...
					<label
						htmlFor='check'
						className='text-[13px] tracking-wide text-stone-700'
					>
				...
				<button
					type='submit'
					className='bg-sky-500 hover:bg-sky-600 duration-150 rounded-lg mt-3 shadow-lg py-3 w-full text-white text-[13px] tracking-wide'
				>
					Login
				</button>
